...
### 変数
Perlの変数には次の3タイプあります: スカラ、配列、 ハッシュです. それぞれのタイプには、自身のシジル(sigil) [^1]があります: それぞれ、$, @ and %。 変数はmyで宣言され、ブロックかファイルの最後までスコープ内に残ります[^2]。

[^1]: 西洋魔術で使われる図形、記号、紋章、線形である
[^1]: 変数が有効な範囲=スコープ ブロックとは{}で括られた処理の内側、ファイルの外側（例えば他のplからは直接参照できない
