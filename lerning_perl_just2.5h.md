...
- リファレンスを解く方法
  - ブレース
  - アロー演算子
    - オブジェクト志向の場合、オブジェクト->メソッド() という使われ方になっている
      - オブジェクトもまた、リファレンスである
...
組込関数が存在していることを知ることに館する最も良いアドバイスは、それらがドキュメントが充実していることです(perldoc)
...
### オブジェクト指向Perl
PerlはOOプログラミングに、あまり適した言語ではありません。 PerlのOO能力は後付のものです。
...
print ref $animal;       # "HASH" # ファレンスのリファレントがどのクラスに属しているかをみつける: blessしてないのでHASH値として返してしまう
bless $animal, "Animal"; # now it is an object of class "Animal" ここでAnimalクラスのオブジェクトになる!

print ref $animal;       # "Animal" # $animalオブジェクトはAnimalクラスに属している
```
注意: 文字通り、どのようなリファレンスも、どのようなクラスにでもblessすることができてしまうので、自分で保証するようcoodingする
- (1)リファレンスが実際にそのクラスのインスタンスとして使われているか
- (2)問題のクラスが存在し、ロードされているか

元のハッシュ変数は、blessしてオブジェクト化しても元どおり参照可能

> print "Animal has ", $animal->{"legs"}, " leg(s)"; # 4leges

同時に`->`オペレータでオブジェクトからメソッドを呼ぶことができる

> $animal->eat("insects", "curry", "eucalyptus");
>
> # Animal::eat($animal, "insects", "curry", "eucalyptus")と同じ

### コンストラクタ
コンストラクタは`クラスメソッド`で、新しいオブジェクトを返します。
コンストラクタが欲しければ、それを宣言するだけです。好きな名前を使えます。
クラスメソッドには、最初の引数として、オブジェクトではなくクラス名が渡ります。このケースでは、"Animal"です:

`コンストラクタにより、別のオブジェクトを生成することができる`
